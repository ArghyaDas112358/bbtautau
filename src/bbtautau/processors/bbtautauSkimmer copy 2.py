...
class bbtautauSkimmer(SkimmerABC):
    ...
    def process(self, events: ak.Array):
        ...
        triggers_for_this_year = HLTs.hlt_list(year=year, hlt_prefix=False)
        HLTVars = {}
        zeros = np.zeros(len(events), dtype="int")
        for trigger in triggers_for_this_year:
        ...
        HLT_triggered = np.any(
            np.array(
                [events.HLT[trigger] for trigger in triggers_for_this_year if trigger in events.HLT.fields]
            ),
            axis=0,
        )
